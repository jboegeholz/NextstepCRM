...
<title>Bewerbung hinzufuegen</title>
</head>
<body>
<%
...
<div id="contents"><html:form action="applicationAdd" focus="name">
	<ul>
	...
	<table>
		...
		<tr>
			<td>Booker</td>
			<td><c:set var="loggedInUser"
				value="${sessionScope.user.username}" /> <html:select
				property="bookerName">

				<c:forEach var="item" items="${applicationAddForm.allUser}">
					<c:choose>
						<c:when test="${item.username eq loggedInUser}">
			...
			<td><html:select property="stageActName">
				<c:forEach var="item" items="${applicationAddForm.allStageActs}">
		...
		<tr>
			...
			<td><html:text readonly="true" value="" property="sendDate" /><a
		...
		<tr>
			...
			<td><html:select property="locationName">
				<option></option>
				<c:forEach var="item" items="${applicationAddForm.allLocations}">
					<option><c:out value="${item.name}" /></option>
				</c:forEach>
			</html:select></td>
			<td>Termin (bei Initiativ)</td>
			<td><html:text readonly="true" value="" property="desiredDate" /><a
		...
		<tr>
			...
			<td><html:select property="eventName">
				<option></option>
				<c:forEach var="item" items="${applicationAddForm.allEvents}">
					<option><c:out value="${item.name}" /></option>
				</c:forEach>
			</html:select></td>
		</tr>
	...
	<table>
		...
		<tr>
			...
			<td><html:text readonly="true" value=""
				property="sendPosterAndFlyerDate" /><a
	...
	<table>
		...
		<tr>
			<td>Gagenvorgabe</td>
			<td><html:text readonly="false" value="0" property="demandedFee" /></td>
			<td>erhaltene Gage</td>
			<td><html:text readonly="false" value="0" property="receivedFee" /></td>
		</tr>
		<tr>
			<td>Unterkunft</td>
			<td><html:text readonly="false" value="0"
	...
	<html:hidden property="do" value="saveApplication" />
</html:form></div>
<div id="footer"><a href="#">Copyright © creatronix 2008</a></div>
</div>
<script language="JavaScript">
  var cal1 = new calendar1(document.forms['applicationAddForm'].elements['sendDate']);
  cal1.year_scroll = true;
  cal1.time_comp = false;
    var cal2 = new calendar1(document.forms['applicationAddForm'].elements['askedForPosterAndFlyerDate']);
  cal2.year_scroll = true;
  cal2.time_comp = false;
   var cal3 = new calendar1(document.forms['applicationAddForm'].elements['sendPosterAndFlyerDate']);
  cal3.year_scroll = true;
  cal3.time_comp = false;
     var cal4 = new calendar1(document.forms['applicationAddForm'].elements['desiredDate']);
